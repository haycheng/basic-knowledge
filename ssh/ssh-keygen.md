...
- dsa：公钥文件中的内容以 ssh-dss 开头（也有说以 ssh-dsa 开头的，但我产生的公钥以 ssh-dss 开头）。
- ecdsa：公钥文件中的内容以 ecdsa-sha2-nistp256 开头。
...
`-C comment` 用于在生成的密钥中添加注释/描述，该注释可以在公钥文件的末尾看到。一般用"your_email@example.com"作为注释。如果没有显式添加注释，则默认注释的形式为“当前用户名@本地主机名”。
