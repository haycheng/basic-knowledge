...
#### 字节序
计算机在存储器中排列字节的方式有两种，分别是大端法（Big Endian, BE）和小端法（Little Endian, LE）。**大端法就是将高位字节放在前面（低地址处），小端法则是将低位字节放在前面（低地址处）**。UTF-32和UTF-16根据字节排序方式的不同，其编码方式分为UTF-32BE、UTF-32LE 和 UTF-16BE、UTF-16LE。
