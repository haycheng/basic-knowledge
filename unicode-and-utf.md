...
那么计算机怎么知道某一个文件到底采用哪一种方式编码？Unicode规范定义，每一个文件的最前面2个字节用来表示编码顺序，并把这两个字节称作”零宽度非换行空间”（ZERO WIDTH NO-BREAK SPACE）。如果文本文件的前两个字节是0xFEFF，表示该文件采用BE方式；如果前两个字节是0xFFFE，则表示该文件采用LE方式。
