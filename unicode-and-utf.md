# 弄懂 Unicode 与 UTF 的关系

> ### Outline:
> 1. 先说明字符集与字符编码的区别
> 1. 再简单叙述下字符集的发展历程
> 1. 然後介绍文本的主角Unicode字符集及UTF编码
> 1. 着重介绍UTF-8编码方式
> 1. 阐述编码中涉及的字节序问题
> 1. 附录中说了下ISO和Unicode联盟，以及一些字符集网站

## 字符集与字符编码
文件通常可分为二进制文件与文本文件两种。二进制文件与文本文件在计算机中都是一些字节流，没有什么区别，但对其解释与使用上有所不同。二进制文件由对应的程序/软件进行解释并使用（如.doc文件由Microsoft Word软件进行处理），而文本文件则不特定于某个程序/软件，而是所有的文本编辑器都可以对一段文本形式的字节流进行处理。

文本编辑器在处理文本形式的字节流时，需要知道这段字节流的编码方式，即将字节与字符对应起来的规则，从而向人们展现正确的文本，这涉及了计算机中的字符集与字符编码的概念。为了在计算机中表示我们平时写的字符（如数字0、1、2，字母a、b、c、A、B、C，标点符号，汉字等），就需要对这些字符进行编码，即在计算机中用特定的字节序列表示这些字符。

**字符集**（Charset）是一组字符（如文字/字母、数字、标点符号、图形符号等）集合，并规定每个字符对应的**码位**（Code Point，或称码点，即每个字符对应的一个唯一的整数ID）。

**字符编码**（Character Encoding）是**把字符集中字符的码位，按一定的规则转换成计算机系统中的字节序列**，便于计算机处理与网络传输。根据编码方式的不同，每个字符对应的字节序列可能是单个或多个字节。

通常来说，**一种字符集只对应一种字符编码**，比如 ASCII、IOS-8859-1、GBK 等，**它们既表示了字符集，又表示了对应的字符编码**。但是Unicode有所不同，它的字符集（Unicode字符集）对应了多种字符编码（UTF-8、UTF-16、UTF-32）。

## 字符集发展小史

#### 美国的 ASCII 字符集
最初，计算机在美国被使用时，由于英文的只有26个字母，算上大小写及阿拉伯数字等其他符号，字符数也不多，于是只需要7个bit（可表示128个字符）就足够了。于是出现了最初的 ASCII 字符集，该字符集用0~127，共128个码位来表示128个字符。在计算机中具体应用起来也比较简单，一个字符仅用一个字节即可表示，且只用到了一个字节的低7位，其最高位始终置为0。
...
ISO-8859 字符集也只用一个字节编码字符，但扩展了 ASCII 中单字节最高位的含义：**字节的最高位为1时，用来标识那些新增的拉丁字母与西里尔字母等；而最高位为0时，含义等同于ASCII字符集**。由于这一系列字符集的标准，是由国际标准化组织(ISO)与国际电工委员会(IEC)联合制定的，所以 ISO-8859 更正式的名称是 ISO/IEC 8859。
...
1. 在台湾、香港与澳门等繁体中文（正体中文）通行区，用的较多的是俗称大五码的 Big5 字符集标准。Big5 字符集使用1~2个字节编码，兼容 ASCII 字符集。
...
- 对于基本平面的字符（即码位在`U+0000~U+FFFF`之间），用2个字节表示。
- 对于辅助平面的字符（即码位在`U+10000~U+10FFFF`之间），则用4个字节表示。

对于基本平面中的字符，编码它们的2个字节值就是字符的Unicode码位。例如，汉字“亮”的Unicode码位为`U+4EAE`，位于基本平面，故其UTF-16编码的为2字节的`0x4EAE`。

在Unicode字符集的码位分布中，有两个叫作 Surrogate 的特殊区段，`0xD800 ~ 0xDBFF`和`0xDC00 ~ 0xDFFF`。这两个区段的值并不用来作为字符的码位，而是用于识别UTF-16编码中，字节是2个还是4个作为一组来表示一个字符。
- `0xD800`(二进制形式为`1101_1000_0000_0000`) ~ `0xDBFF`(二进制形式为`1101_1011_1111_1111`)：该区段高位的6 bit固定为`110110`，低位的10 bit变化范围为`0000000000~1111111111`。
- `0xDC00`(二进制形式为`1101_1100_0000_0000`) ~ `0xDFFF`(二进制形式为`1101_1111_1111_1111`)：该区段高位的6 bit固定为`110111`，低位的10 bit变化范围为`0000000000~1111111111`。

辅助平面的码位范围是`U+010000`到`U+10FFFF`，故有`0xFFFFF+1=2^20`个码位，表示这些字符只需20个bit。对于任意一个辅助平面中的码位，UTF-16编码步骤如下：
...
计算机在处理UTF-16的编码字节时，以2个字节为一组。当前字节组若不在`0xD800 ~ 0xDBFF`之间，则该字节组的值即为一个Unicode码位，表示一个字符；若在`0xD800 ~ 0xDBFF`之间，则该字节组与後面的一个2字节组一起表示一个字符，其码位由前後字节组的低10个bit组合、运算後得到。

#### 辅助平面字符的UTF-16编码示例
汉字“𠄯”的Unicode码位是`0x2012F`，位于辅助平面，故其UTF-16需要4字节编码。将其减去`0x010000`後为`0x1012F`，其二进制形式为`1_0000_0001_0010_1111`，补足成20 bit後为`00010_00000_01001_01111`。前10 bit加上前缀`110110`後为`1101_1000_0100_0000`，即高位的2字节组H=`0xD840`；後10 bit加上前缀`110111`後为`1101_1101_0010_1111`，即低位的2字节组L=`0xDD2F`。
...
>> 如汉字“亮”的Unicode码位为U+4EAE（10进制值为20142，2进制值为1001110_10101110）（加下划线是为了阅读方便）。转换成UTF-8编码时，先将低6位101110单独取出，加上10前缀後为10101110，即16进制的0xAE；再将中间6位111010取出，加上前缀10後为10111010，即16进制的0xBA；最後剩余3位100，补足成4位0100，再加上前缀1110後为11100100，即16进制的0xE4。因此汉字“亮”在UTF-8编码规则中为3个字节，最高字节为0xE4，中间字节为0xBA，最低字节为0xAE。

## 字节序问题
由于UTF-8的基本处理单元为一个字节（也就是一次处理一个字节），所以处理器在处理的时候，就不需要考虑这一个字节的存储是在高位还是在低位，因为它是独立的。而UTF-32和UTF-16编码方式，分别以4字节和2字节为基本处理单元，因此就有字节顺序的问题。

计算机在存储器中排列字节的方式有两种，分别是大端法（Big Endian, BE）和小端法（Little Endian, LE）。**大端法就是将高位字节放在前面（低地址处），小端法则是将低位字节放在前面（低地址处）**。为了标识字符编码的字节排列顺序，会在一段编码的起始处添加一个字节顺序标记（Byte-Order Mark，BOM）。

这个BOM字符在Unicode中的码位是`0xFEFF`（10进制为65279），叫做”零宽度非换行空格”（Zero Width No-Break Space，ZWNBS）。叫这个名称是因为这个字符最初是用于文字排版的，是个不可打印、不可见的字符，主要在不用空格分隔的文字中，用于标识在某个位置不能换行。ZWNBS字符的原本的功能後来由字符Word joiner（WJ，Unicode码位是U+2060）代替，而ZWNBS则专用于字节流起始处表示字节序（ZWNBS若不出现在起始处，则仍表示零宽非换行空格，但不推荐这么使用）。

具体地，字节序列起始处的BOM值若为`0xFEFF`，则表示字节是按大端法排列的；若为`0xFFFE`，则表示字节是按小端法排列的。根据上述各编码方式的说明，可得`0xFEFF`的编码值如下所示：

编码方式|大端法（BE）|小端法（LE）
-|-|-
UTF-8|`0xEFBBBF`|`0xEFBFBE`
UTF-16|`0xFEFF`|`xFFFE`
UTF-32|`0x0000FEFF`|`xFFFE0000`

因此：
- 对于UTF-16，若起始两个字节为`0xFEFF`，则表示大端法的UTF-16BE编码；若起始两个字节为`0xFFFE`，则表示小端法的UTF-16LE编码。
- 对于UTF-32，若起始两个字节为`0x0000FEFF`，则表示大端法的UTF-32BE编码；若起始两个字节为`0xFFFE0000`，则表示小端法的UTF-32LE编码。
- 对于UTF-8来说，不存在大端或小端法。但Windows系统中的一些编辑器，也会在UTF-8编码字节序列起始处加上`0xEFBBBF`，仅仅表示字符是按UTF-8编码的。PHP引擎或浏览器在处理这种字节序列时可能会出错，因此在Windows上开发时最好选择不带BOM的UTF-8格式存储。
...
#### 字符集相关的网站
- [Unicode Consortium](https://home.unicode.org/basic-info/overview/)
...
1. [Unicode学习笔记](https://blog.csdn.net/worm0527/article/details/70306125)
