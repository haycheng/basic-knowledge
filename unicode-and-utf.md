...
## Unicode字符集
Unicode字符集为每个字符分配一个唯一的数字编号即码位，码位的范围从 0x000000 到 0x10FFFF。这些码位分为17个平面（Plane），每平面拥有65536个码位，共65536 * 17 = 1114112个（码位范围为 0x0000~0x10FFFF）。`Unicode`目前只用了少数几个平面，收录了约15万个字符。
