...
UTF-16|0xFEFF|0xFFFE
UTF-32|0x0000FEFF|0xFFFE0000


若按高位在前，码位`0xFEFF`的UTF-8编码为0xEF、0xBB、0xBF，UTF-16编码为，UTF-32编码为。
UTF-32和UTF-16根据字节排序方式的不同，其编码方式分为UTF-32BE、UTF-32LE 和 UTF-16BE、UTF-16LE 编码方式。

那么计算机怎么知道某一个文件到底采用哪一种方式编码？Unicode规范定义，每一个文件的最前面2个字节用来表示编码顺序，并把这两个字节称作”零宽度非换行空间”（ZERO WIDTH NO-BREAK SPACE）。如果文本文件的前两个字节是`0xFEFF`，表示该文件采用BE方式；如果前两个字节是`0xFFFE`，则表示该文件采用LE方式。


BOM（，字节序标记）是Unicode码点U+FEFF。它被定义来放在一个UTF-16文件的开头，如果字节序列是FEFF那么这个文件就是大端序，如果字节序列是FFFE那么这个文件就是小端序。
