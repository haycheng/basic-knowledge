...
计算机在存储器中排列字节的方式有两种，分别是大端法（Big Endian, BE）和小端法（Little Endian, LE）。**大端法就是将高位字节放在前面（低地址处），小端法则是将低位字节放在前面（低地址处）**。字节顺序标记（Byte-Order Mark，BOM），就是放在一段字符编码开头，用来标识字节高低位之间顺序的。在Unicode中，有个特殊的码位`0xFEFF`（10进制为65279），叫做”零宽度非换行空格”（Zero Width No-Break Space，ZWNBS），BOM就是用这个ZWNBS来表示後面的字节顺序是BE还是LE。
