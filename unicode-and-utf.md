...
对于基本平面中的字符，编码它们的2个字节值就是字符的Unicode码位。例如，汉字“中”的Unicode码位为U+4E2D，位于基本平面，故其UTF-16编码的高位H=4E，地位L=2D。
...
1. 将码位减去辅助平面的起始码位0x010000。
1. 得到的值若不足20 bit，则将其补足成20 bit。
1. 将这20个bit分成两半，前10位加上前缀110110，使其值位于0xD800 ~ 0xDBFF之间，作为高位的2字节组。
1. 后10位加上前缀110111，使其值位于在0xDC00~0xDFFF之间，作为低位的2字节组。
...
#### UTF-16 编码示例
汉字“𠄯”的Unicode码位是0x2012F，位于辅助平面。将其减去0x010000後为0x1012F，其二进制形式为1_0000_0001_0010_1111，补足成20 bit後为00010_00000_01001_01111。前10 bit加上前缀110110後为 1101_1000_0100_0000，即高位的2字节组H=0xD840；後10 bit加上前缀110111後为1101_1101_0010_1111，即低位的2字节组L=0xDD2F。
