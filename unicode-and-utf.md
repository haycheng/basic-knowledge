...
计算机在存储器中排列字节的方式有两种，分别是大端法（Big Endian, BE）和小端法（Little Endian, LE）。**大端法就是将高位字节放在前面（低地址处），小端法则是将低位字节放在前面（低地址处）**。字节顺序标记（Byte-Order Mark，BOM），就是放在一段字符编码开头，用来标识字节高低位之间顺序的。在Unicode中，有个特殊的码位`0xFEFF`，叫做”零宽度非换行空格”（ZERO WIDTH NO-BREAK SPACE，ZWNBS），BOM就是用这个ZWNBS来表示後面的字节顺序是BE还是LE。

那具体是怎么实施的呢？字节序列起始处的BOM值若为`0xFEFF`，则表示字节是按大端法排列的；若为`0xFFFE`，则表示字节是按小端法排列的。
