...
文本编辑器对处理文本形式的字节流时，需要知道这段字节流的编码方式，即将字节与字符对应起来的规则，从而向人们展现正确的文本，这涉及了计算机中的字符集与字符编码的概念。为了在计算机中表示我们平时写的字符（如数字0、1、2，字母a、b、c、A、B、C，标点符号，汉字等），就需要对这些字符进行编码，即在计算机中用特定的字节序列表示这些字符。
...
辅助平面的码位范围是`U+010000`到`U+10FFFF`，故有`0xFFFF+1=2^20`个码位，表示这些字符只需20个bit。对于任意一个辅助平面中的码位，UTF-16编码步骤如下：
